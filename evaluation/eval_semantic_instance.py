...
# Vendored into CDIS/evaluation/ from OVMap evaluation_scannet. Self-contained:
# depends only on numpy (+ torch for the __main__ demo). The class-agnostic AP
# math (IoU matching, overlap thresholds, AP/AP50/AP25 integration, *1000 GT
# encoding + ignored-class handling) is kept IDENTICAL to the original.
    ...
    for di, (min_region_size, distance_thresh, distance_conf) in enumerate(
        ...
        for oi, overlap_th in enumerate(overlaps):
            ...
            for li, label_name in enumerate(CLASS_LABELS):
                # Perform evaluation for class-agnostic or class-specific logic here
...
def evaluate_matches_class_agnostic(matches, class_agnostic=False):
    overlaps = opt['overlaps']
    min_region_sizes = [opt['min_region_sizes'][0]]
    dist_threshes = [opt['distance_threshes'][0]]
    dist_confs = [opt['distance_confs'][0]]

    # In class-agnostic mode, only one "class"
    if class_agnostic:
        num_classes = 1
        class_list = ['class_agnostic']
    else:
        num_classes = len(CLASS_LABELS)
        class_list = CLASS_LABELS

    ap = np.zeros((len(dist_threshes), num_classes, len(overlaps)), float)
    ar = np.zeros((len(dist_threshes), num_classes, len(overlaps)), float)
    rc = np.zeros((len(dist_threshes), num_classes, len(overlaps)), float)
    ...
    for di, (min_region_size, distance_thresh, distance_conf) in enumerate(zip(min_region_sizes, dist_threshes, dist_confs)):
        ...
        for oi, overlap_th in enumerate(overlaps):
            ...
            for li, label_name in enumerate(class_list):
                y_true = np.empty(0)
                y_score = np.empty(0)
                hard_false_negatives = 0
                has_gt = False
                ...
                for m in matches:
                    ...
                    for (gti, gt) in enumerate(gt_instances):
                        # If class-agnostic, don't use the label_id for the ground truth, just treat as one class
                        if class_agnostic:
                            gt_category_names[m].add('class_agnostic')
                        else:
                            # Otherwise, use the label ID as in class-aware mode
                            try:
                                gt_category_names[m].add(ID_TO_LABEL[gt['label_id']])
                            except KeyError:
                                print(f"Warning: gt['label_id'] {gt['label_id']} not found in ID_TO_LABEL")
                                continue  # Skip this instance if the label is not found
                        ...
                        if not found_match:
                            hard_false_negatives += 1
                        if found_match:
                            matched_predictions_category_names[di][oi][m].add('class_agnostic' if class_agnostic else ID_TO_LABEL[gt['label_id']])
...
def assign_instances_for_scan(pred: dict, gt_file: str, class_agnostic=False):
    pred_info = make_pred_info(pred)
    try:
        gt_ids = util_3d.load_ids(gt_file)
    except Exception as e:
    ...
    # Get ground truth instances based on class_agnostic flag
    gt_instances = util_3d.get_instances(gt_ids, VALID_CLASS_IDS, CLASS_LABELS, ID_TO_LABEL, class_agnostic=class_agnostic)
    
    # Initialize gt2pred and pred2gt based on the mode
    gt2pred = deepcopy(gt_instances)
    for label in gt2pred:
        for gt in gt2pred[label]:
            gt['matched_pred'] = []

    pred2gt = {'class_agnostic': []} if class_agnostic else {label: [] for label in CLASS_LABELS}
    ...
    for uuid in pred_info:
        ...
        if class_agnostic:
            label_name = 'class_agnostic'  # Ignore the actual label in class-agnostic mode
        else:
            if label_id not in ID_TO_LABEL:
                continue
            label_name = ID_TO_LABEL[label_id]

        # Convert to binary mask and count vertices
        pred_mask = pred_info[uuid]['mask']
        pred_mask = np.not_equal(pred_mask, 0)
        num = np.count_nonzero(pred_mask)
        if num < opt['min_region_sizes'][0]:
        ...
        pred_instance = {
            ...
            'label_id': label_id if not class_agnostic else 0,  # Ignore label in class-agnostic mode
            'vert_count': num,
            'confidence': conf,
...
def write_result_file(avgs, filename):
    ...
    output_dir = os.path.dirname(filename)  # Extract the directory from the file path
...
def evaluate(preds: dict, gt_path: str, output_file: str, dataset: str = "scannet", class_agnostic=False):
    global DATASET_NAME, CLASS_LABELS, VALID_CLASS_IDS, ID_TO_LABEL, LABEL_TO_ID, opt
    #pdb.set_trace()
    ...
    if class_agnostic:
        DATASET_NAME = "class_agnostic"
        CLASS_LABELS = ['object']  # Single label for all objects
        VALID_CLASS_IDS = np.array([1])  # Arbitrary ID for the single object class
    ...
    for i, (k, v) in enumerate(preds.items()):
        ...
        matches_key = os.path.abspath(gt_file)
        gt2pred, pred2gt = assign_instances_for_scan(v, gt_file, class_agnostic=class_agnostic)
        matches[matches_key] = {'gt': gt2pred, 'pred': pred2gt}
        sys.stdout.write("\rscans processed: {}".format(i + 1))
        sys.stdout.flush()

    print('')
    ap_scores, ar_scores, rc_scores, pcdc_scores = evaluate_matches_class_agnostic(matches, class_agnostic=class_agnostic)
    avgs = compute_averages(ap_scores)
    ar_avgs = compute_averages_ar(ar_scores)
    rc_avgs = compute_averages_rc(rc_scores)
    pcdc_avgs = compute_averages_pcdc(pcdc_scores)
