...
try:
    ...
    PlyData, PlyElement = None, None  # only needed for read_mesh_vertices (unused in eval)
...
def export_instance_ids_for_eval(filename, label_ids, instance_ids, class_agnostic=False):
    assert label_ids.shape[0] == instance_ids.shape[0]
    output_mask_path_relative = 'pred_mask'
    name = os.path.splitext(os.path.basename(filename))[0]
    output_mask_path = os.path.join(os.path.dirname(filename), output_mask_path_relative)
    ...
    with open(filename, 'w') as f:
        for idx, inst_id in enumerate(insts):
            ...
            if class_agnostic:
                label_id = 0  # Assign all instances to class 0 in class-agnostic mode
            else:
                label_id = label_ids[loc[0][0]]
...
def get_instances(ids, class_ids, class_labels, id2label, class_agnostic=False):
    instances = {}
    
    if class_agnostic:
        instances['class_agnostic'] = []  # All instances will go into this class
    else:
        for label in class_labels:
            instances[label] = []
    ...
    for id in instance_ids:
        ...
        if class_agnostic:
            instances['class_agnostic'].append(inst.to_dict())  # Treat all instances as class agnostic
        elif inst.label_id in class_ids:
            instances[id2label[inst.label_id]].append(inst.to_dict())
