...
def visualize_3d_instances(pcd_list, intensity_dict, scene_pcd_path):
...
def get_cropformer(image, mask_generator, score_threshold=0.7):
    """Run CropFormer and paint the kept masks into a single instance label map.

    ``score_threshold`` comes from ``vfm_config.cropformer_config.confidence-threshold``.
    Masks are painted in ascending score order, so each pixel ends up owned by the
    highest-scoring mask covering it.
    """
    ...
    selected_indexes = (pred_scores >= score_threshold)
...
def color_reset(color_map):
    """Reassign new random colors to all existing IDs except -1."""
    for id_ in color_map.keys():
        if id_ == -1:
            continue  # Keep default black for -1
        # Assign a new random color (not black or red)
        while True:
            color = [random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)]
            if color != [0, 0, 0] and color != [0, 0, 255]:
                color_map[id_] = color
                break

def get_color_for_id(id_, color_map):
...
def visualize_warped_masks_over_time(current_mask, warped_masks, color_map):
    ...
    def assign_consistent_colors(mask):
        ...
        for id_ in np.unique(mask):
            color = get_color_for_id(id_, color_map)
    ...
    concatenated_result = np.concatenate(visualizations[::-1], axis=1)
    
    return concatenated_result


def assign_consistent_colors_with_id_text(mask, color_map):
    """Assign consistent RGB colors and draw instance ID text for each unique ID."""
    colored_mask = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.uint8)
    ...
    for id_ in unique_ids:
        ...
        # Compute the centroid of the instance
...
def visualize_inst_matching_over_time(inst_match_list, color_map, max_view=5, step=1):
    """
    Visualize instance matching results with step sampling:
    - Takes up to `max_view` frames, sampled every `step` frames.
    - Colors are consistent for instance IDs.
    """
    if len(inst_match_list) == 0:
        return None

    # Sample with step and take max_view frames
    sampled_masks = inst_match_list[-(max_view * step)::step]
    recent_masks = [frame["mask_2d"] for frame in sampled_masks]

    # Convert each mask to colored visualization
    visualizations = [assign_consistent_colors_with_id_text(mask, color_map) for mask in recent_masks]
