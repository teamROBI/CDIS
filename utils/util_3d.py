...
def build_scene_point_cloud(args, file_names, scene_name, vis_final=False):
    dense_scene_pcd_dir = os.path.join(args.data.data_root, "temp_dense_pcd")
    ...
    intrinsic_path = join(args.data.scans_2d_path, scene_name, 'intrinsics', 'intrinsic_depth.txt')
    depth_intrinsic = np.loadtxt(intrinsic_path)

    # Scene point cloud: `data.pcd_path` already points at the right split. Accept either the
    # ScanNet mesh (.ply) or a preprocessed tensor (.pth with a "coord" entry).
    ply_path = join(args.data.pcd_path, scene_name + ".ply")
    pth_path = join(args.data.pcd_path, scene_name + ".pth")
    if os.path.exists(ply_path):
        voxelized_pcd = o3d.io.read_point_cloud(ply_path)
    elif os.path.exists(pth_path):
        voxelized_pcd = o3d.geometry.PointCloud()
        voxelized_pcd.points = o3d.utility.Vector3dVector(torch.load(pth_path)["coord"])
    else:
        raise FileNotFoundError(
            f"No scene point cloud for {scene_name}: expected {ply_path} or {pth_path} "
            f"(see data.pcd_path in the config and docs/DATA.md)."
        )
    ...
    if args.matching_2d.image_iter == 1 and args.dataset_type == "scannetv2":
        image_iter = 2 #10
    else:
        image_iter = args.matching_2d.image_iter
        
    for file_name in tqdm(file_names[::image_iter]): #2:30
        pose = join(args.data.scans_2d_path, scene_name, 'pose', file_name[0:-4] + '.txt')
        depth = join(args.data.scans_2d_path, scene_name, 'depth', file_name[0:-4] + '.png')
...
def backproject_to_3d(mask, depth, intrinsics, depth_scale=1000.0):
    # Handle depth scale
    if depth.dtype in (np.float64, np.float32) and np.max(depth) < 100:
        depth_scale = 1.0

    # Get only valid indices (no full meshgrid)
    valid_idx = np.where((mask >= 0) & (depth > 0))
    y_coords, x_coords = valid_idx
    z = depth[y_coords, x_coords] / depth_scale
    ...
    x = (x_coords - cx) * z / fx
    y = (y_coords - cy) * z / fy
    points_3d = np.column_stack((x, y, z))  # shape (N, 3)

    return points_3d, mask[y_coords, x_coords], valid_idx
...
def transform_points(points_3d, pose):
    ...
    return (points_3d_h @ pose.T)[:, :3]

def project_to_2d(points_3d, intrinsics):
    # Multiply directly (N,3) @ (3,3).T = (N,3)
    pts = points_3d @ intrinsics.T
    # Normalize
    pts[:, 0] /= pts[:, 2]
    pts[:, 1] /= pts[:, 2]
    return pts[:, :2].T
